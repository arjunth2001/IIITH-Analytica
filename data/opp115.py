...
def generate_dataset():
    ...
    mode = merged.groupby(['policy_id', 'segment_id']).agg(
...
def load_annotations():
    ...
    for f in glob('opp-115/annotations/*.csv'):
        a = pd.read_csv(f, sep=',', header=None, names=[
                        'annotation_id', 'batch_id', 'annotator_id', 'policy_id', 'segment_id', 'data_practice', 'attributes', 'date', 'url'])
        a['policy_id'] = f[20:-4].split('_')[0]
        a.drop(['annotation_id', 'batch_id', 'annotator_id',
               'date', 'url'], axis=1, inplace=True)
...
if __name__ == "__main__":
    data = load()
    data.to_csv("opp115.csv", index=False)
